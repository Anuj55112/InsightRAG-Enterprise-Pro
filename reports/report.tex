...
\subsection{Semantic Cache Layer}
To optimize query latency and API expenses, a semantic cache layer is implemented. Incoming queries are mapped to the embedding space $E_Q$ and evaluated against historic queries $E_{C_i}$ using cosine similarity:
\begin{equation}
\text{Similarity}(E_Q, E_{C_i}) = \frac{E_Q \cdot E_{C_i}}{\|E_Q\|_2 \|E_{C_i}\|_2} \ge \tau
\end{equation}
where $\tau = 0.92$ is the similarity threshold. If this condition is met, the cached response is served directly, bypassing both vector retrieval and LLM call paths. In production logs, this caching mechanism has demonstrated a verified \textbf{35\% reduction in LLM token usage} while maintaining query precision.
